...
{/* <g2-chart id="g2Chart"
    ...
    data-chart-config='{"xScale":{"name":"交易日期"},"yScale":[{"name":"融资余额(亿元)","showAxis":true,"isLabel":false},{"name":"融券余额(亿元)","showAxis":true,"isLabel":false}]}'
></g2-chart> */}
function drawBarChart(container, chartData, config) {
    ...
    if (config) {
        ...
        config.yScale.forEach((item) => {
            chart.interval().position(`${config.xScale.name}*${item.name}`).label(item.isLabel ? item.name : '');
            // 是否显示对应的坐标轴
            if (!item.showAxis) {
